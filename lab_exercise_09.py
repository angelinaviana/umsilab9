...
def board_starship(starship, people):
    ...
    for person, intruder in people:
        if intruder and 'intruder' in starship:
            starship['intruder'].append(person)
        elif intruder and 'intruder' not in starship:
...
def capture_starship(starship_attacker, starship_captured):
    ...
    if 'primary_docking_bay' not in starship_attacker:
        starship_attacker['primary_docking_bay'] = {'docked': [starship_captured]}
    else:
        starship_captured['primary_docking_key'].append(starship_captured)
...
def main():
    ...
    #swapi_new_hope = get_swapi_resource(f'{ENDPOINT}/films', params={'search': 'new hope'})['results']
    swapi_new_hope = get_swapi_resource(f'{ENDPOINT}/films', params={'search': 'new hope'})['results'][0]
    swapi_new_hope_title = swapi_new_hope['title']
    ...
    swapi_rebel_blockade = get_swapi_resource(f'{ENDPOINT}/starships', params={'search': 'CR90 corvette'})['results'][0]

    rebel_blockade = create_starship(swapi_rebel_blockade)

    swapi_stardestroyer = get_swapi_resource(f'{ENDPOINT}/starships', params={'search': 'destroyer'})['results'][0]
    ...
    swapi_r2d2 = get_swapi_resource(f'{ENDPOINT}/people', params={'search': 'R2-D2'})['results'][0]

    r2d2 = create_person(swapi_r2d2)

    swapi_c3po = get_swapi_resource(f'{ENDPOINT}/people', params={'search': 'C-3PO'})['results'][0]
